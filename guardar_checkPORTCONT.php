...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!$stmt->execute()) {
        die("Error en la consulta port_cont: " . $stmt->error);
    }
    ...
    $stmt = $conn->prepare("INSERT INTO datos_portcont (unidad, placas, fecha) VALUES (?, ?, ?)");
    $stmt->bind_param("sss", $unidad, $placas, $fecha);
    if (!$stmt->execute()) {
        die("Error en la consulta datos_portcont: " . $stmt->error);
    }
    ...
    $stmt = $conn->prepare("INSERT INTO port_porcentaje (eje, a1, a2, a3, a4, b1, b2, b3, b4) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("issssssss", $eje, $a1, $a2, $a3, $a4, $b1, $b2, $b3, $b4);

    // Insertar observaciones
    $observaciones = sanitizeInput($_POST['observaciones'] ?? '');

    $stmt = $conn->prepare("INSERT INTO observ_port (observaciones) VALUES (?)");
    $stmt->bind_param("s", $observaciones);
    if (!$stmt->execute()) {
        die("Error en la consulta observ_port: " . $stmt->error);
    }
    ...
    $stmt = $conn->prepare("INSERT INTO firmas_port (mantenimiento, operador, vigilancia) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $mantenimiento, $operador, $vigilancia);
    if (!$stmt->execute()) {
        die("Error en la consulta firmas_port: " . $stmt->error);
    }
