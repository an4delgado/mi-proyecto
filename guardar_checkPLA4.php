...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO datos_extcuatro (unidad, placas, fecha) VALUES (?, ?, ?)";
    ...
    $sql = "INSERT INTO plataforma_extcuatro (frenos, suspension, neumaticos, luces_intermitentes, luces_traseras,
    ...
    $d1 = sanitizeInput($_POST['d1'] ?? '');
    $d2 = sanitizeInput($_POST['d2'] ?? '');
    $d3 = sanitizeInput($_POST['d3'] ?? '');
    $d4 = sanitizeInput($_POST['d4'] ?? '');


    $sql = "INSERT INTO extcuatro_porcentaje (ejes,a1, a2, a3, a4, b1, b2, b3, b4, c1, c2, c3, c4,d1,d2,d3,d4)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssssssssss",$ejes, $a1, $a2, $a3, $a4, $b1, $b2, $b3, $b4, $c1, $c2, $c3, $c4,$d1,$d2,$d3,$d4);
    $stmt->execute();

    // Insertar observaciones
    $observaciones = sanitizeInput($_POST['observaciones'] ?? '');
    $sql = "INSERT INTO extcuatro_observaciones (observaciones) VALUES (?)";
    $stmt = $conn->prepare($sql);
    ...
    $sql = "INSERT INTO firmas_extendiblecuatro (mantenimiento, operador, vigilancia) VALUES (?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
